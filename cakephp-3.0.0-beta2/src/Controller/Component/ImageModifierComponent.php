...
* @static
*/
class ImageModifierComponent extends Component{
     ...
     **/
    public function get($src, $params = array(), $sendCachePath = true, $config = null) {
        ...
        foreach($params as $key => $value) {
            if( in_array($key, ImageModifierComponent::$transformParams)) {
                $chaine .= $key.$value;
            }
            if( in_array($key, ImageModifierComponent::$attributeParams)) {
        ...
        if( $pendingTransforms && is_file($srcPath.$src) && !is_file($cachePath.$cacheName) ) {
            $imgCachePath = Configure::read('App.imgCachePath') ;
            self::transformImage($srcPath.$src, $imgCachePath, $cacheName, $params);
        }
    ...
    static public function computeUrlFilePath($config=null)
    {
        ...
        //$config = & array('src_url' => null, 'src_path' => null, 'cache_url' => null, 'cache_path' => null);
       /* array (
            'src' => 'http://comparateur-lingerie.fr/cache/images/07ba25a0e1c7b8922c20058982dd7780.jpg',
            'cache_path' => '/home/ornythorink/linge/linge/linge/www/cache/images/07ba25a0e1c7b8922c20058982dd7780.jpg',
        )*/
        /*array (
            'src' => 'http://localhost/tmp/cache/images/',
            'cache_path' => '/var/www/html/projects/cakephp-3.0.0-beta2/lingerie/tmp/cache/images/',
        );*/
        $imgCachePath = Configure::read('App.imgCachePath') ;
        $imgCacheUri  = Configure::read('App.imgCacheUri') ;
        $srcPath = '';
        $srcUri  = '';

        $cachePath =  $imgCacheUri;
        $cacheUri = $imgCachePath ;
